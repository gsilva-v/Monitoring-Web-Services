...
static void	pretty_log_ok(DNS_Monitoring *monitor, char *stamp){
	if (!conf.simplified)
		dprintf(conf.log_fd, PDLOGOK, stamp, monitor->name, monitor->url, \
...
static void	pretty_log_ko(DNS_Monitoring *monitor, char *stamp){
	if (!conf.simplified)
		dprintf(conf.error_log_fd, PDLOGKO, stamp, monitor->name, monitor->url, \
		monitor->protocol, monitor->dns_server);
	else
...
static void	handle_log_error(DNS_Monitoring *monitor, char *stamp){
	...
	if (conf.pretty){
		...
		if (!conf.simplified)
			dprintf(conf.error_log_fd, DLOGKO, stamp, monitor->name, monitor->url, \
			monitor->protocol, monitor->dns_server);
		else
...
static void	handle_log_success(DNS_Monitoring *monitor, char *stamp){
	if (conf.pretty){
		...
		if (!conf.simplified)
			dprintf(conf.log_fd, DLOGOK , stamp, monitor->name, monitor->url, \
			monitor->protocol, monitor->latency);
		else
